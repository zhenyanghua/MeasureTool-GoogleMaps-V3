  ...
  _createClass(MeasureTool, [{
    ...
    value: function _updatePath() {
      ...
      var pathAux = this._pathAux.selectAll("path").data(this._geometry ? this._geometry.paths : []).attr("class", "aux-path").attr('d', gmPath).on('mousemove', function (d) {
        self._pointSegment = _helper2.default.getPointOnPath([_d3Selection.event.offsetX, _d3Selection.event.offsetY], self._pathsSegementsInSvg);
        var point = self._pointSegment.touchPoint;
      ...
      var pathsSegments = this._geometry ? this._geometry.pathSegments : [];
      this._pathsSegementsInSvg = pathsSegments.map(function (segment) {
        return [_this4._projectionUtility.latLngToSvgPoint(segment[0]), _this4._projectionUtility.latLngToSvgPoint(segment[1])];
      });
    }
  }, {
    key: '_onDragCircle',
    ...
    value: function _onDragPath() {
      ...
      var pathDrag = (0, _d3Drag.drag)().on('drag', function (d, i) {
        ...
        if (!isDragged) {
          isDragged = true;
          _this5._geometry.insertWayPoints(_this5._pointSegment.segmentIndex + 1, _this5._projectionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]));

          _this5._pathBase.selectAll("path").data(_this5._geometry ? _this5._geometry.paths : []);
          _this5._pathAux.selectAll("path").data(_this5._geometry ? _this5._geometry.paths : []);
        }
        _this5._updateHoverCirclePosition(_d3Selection.event.x, _d3Selection.event.y);
        var gmTransform = (0, _d3Geo.geoTransform)({
          point: function point(x, y) {
            var point = [];
        ...
        var gmPath = (0, _d3Geo.geoPath)().projection(gmTransform);
        // update path
        _this5._pathBase.selectAll("path").attr('d', gmPath);
        _this5._pathAux.selectAll("path").attr('d', gmPath);
      });

      pathDrag.on('start', function (d) {
        _d3Selection.event.sourceEvent.stopPropagation();
        _this5._hoverCircle.select("circle").attr('class', "cover-circle");
        self._disableMapScroll();
      });
      ...
      pathDrag.on('end', function (d) {
        self._enableMapScroll();
        if (isDragged) {
          _this5._geometry.updateWayPoints(_this5._pointSegment.segmentIndex + 1, _this5._projectionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]));
          _this5._hideHoverCircle();
          _this5._overlay.draw();
          isDragged = false;
        }
        _this5._hoverCircle.select("circle").attr('class', "grey-circle");
      });
