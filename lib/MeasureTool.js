  ...
  _createClass(MeasureTool, [{
    ...
    value: function _onDragLine() {
      ...
      var lineDrag = (0, _d3Drag.drag)().on('drag', function (d, i) {
        if (!isDragged) {
          isDragged = true;
          _this5._geometry.insertNode(i + 1, _this5._projectionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]));
          _this5._updateLine();
        }
        _this5._updateHoverCirclePosition(_d3Selection.event.x, _d3Selection.event.y);
        _this5._updateLinePosition(_this5._linesBase, i + 1);
        _this5._updateLinePosition(_this5._linesAux, i + 1);
      });

      lineDrag.on('start', function () {
        _d3Selection.event.sourceEvent.stopPropagation();
        _this5._hoverCircle.select("circle").attr('class', "cover-circle");
        _this5._disableMapScroll();
      });

      lineDrag.on('end', function (d, i) {
        _this5._enableMapScroll();
        if (isDragged) {
          _this5._geometry.updateNode(i + 1, _this5._projectionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]));
