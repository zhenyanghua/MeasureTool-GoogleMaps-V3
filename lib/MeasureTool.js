  ...
  _createClass(MeasureTool, [{
    ...
    value: function _updateNodeText() {
      ...
      var text = this._nodeText.selectAll("text").data(this._geometry ? this._geometry.nodes : []).attr('class', 'node-measure-text').attr('text-anchor', 'middle').attr('dominant-baseline', 'text-after-edge').attr('x', function (d) {
        ...
        return _helper2.default.computePathLength(_this6._geometry.nodes.slice(0, i + 1));
      });
    ...
    value: function _updateSegmentTextPosition(index) {
      ...
      if (index < this._geometry.lines.length) {
        this._segmentText.select('text:nth-child(' + (index + 1) + ')').attr('transform', function (d) {
          ...
          return _helper2.default.computeLengthBetween(_this8._projectionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]), d[1]);
        });
      }
      if (index > 0) {
        this._segmentText.select('text:nth-child(' + index + ')').attr('transform', function (d) {
          ...
          return _helper2.default.computeLengthBetween(d[0], _this8._projectionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]));
        });
      }
    }
  }, {
    ...
    value: function _updateNodeTextPosition(index) {
      ...
      followingNodes.text(function (d, i) {
        return _helper2.default.computePathLength([].concat(_toConsumableArray(_this9._geometry.nodes.slice(0, index)), [_this9._projectionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y])], _toConsumableArray(_this9._geometry.nodes.slice(index + 1, index + 1 + i))));
      });
    }
  }, {
