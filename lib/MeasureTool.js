...
var MeasureTool = function () {
  ...
  _createClass(MeasureTool, [{
    ...
    value: function _updateNodeText() {
      ...
      var text = this._nodeText.selectAll("text").data(this._geometry ? this._geometry.nodes : []).attr('class', 'node-measure-text').attr('text-anchor', 'middle').attr('dominant-baseline', 'text-after-edge').attr('x', function (d) {
        return _this6._projectionUtility.latLngToSvgPoint(d)[0];
      }).attr('y', function (d) {
        return _this6._projectionUtility.latLngToSvgPoint(d)[1];
      }).text(function (d, i) {
        return _this6._helper.computePathLength(_this6._geometry.nodes.slice(0, i + 1));
      });

      text.enter().append('text').attr('class', 'node-measure-text').attr('text-anchor', 'middle').attr('dominant-baseline', 'text-after-edge').attr('x', function (d) {
        return _this6._projectionUtility.latLngToSvgPoint(d)[0];
      }).attr('y', function (d) {
        return _this6._projectionUtility.latLngToSvgPoint(d)[1];
      }).text(function (d, i) {
    ...
    value: function _updateNodeTextPosition(index) {
      ...
      this._nodeText.select('text:nth-child(' + (index + 1) + ')').attr('x', _d3Selection.event.x).attr('y', _d3Selection.event.y);
