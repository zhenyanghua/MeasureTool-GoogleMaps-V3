...
var Helper = function () {
  ...
  _createClass(Helper, [{
    ...
    value: function computeLengthBetween(p1, p2) {
      return this._formatLength(google.maps.geometry.spherical.computeDistanceBetween(new google.maps.LatLng(p1[1], p1[0]), new google.maps.LatLng(p2[1], p2[0])) * this._lengthMultiplier);
    }
  }, {
    ...
    value: function computeArea(points) {
      return this._formatArea(google.maps.geometry.spherical.computeArea(points.map(function (p) {
    ...
    key: 'computePixelLength',
    value: function computePixelLength(p1, p2) {
...
var MeasureTool = function () {
  ...
  function MeasureTool(specification) {
  ...
  _createClass(MeasureTool, [{
    ...
    value: function _checkClick(mouseEvent) {
      ...
      if (this._nodeCircles.selectAll('circle[r="6"]').size() == 0 && !this._hoverCircle.select("circle").attr('cx')) {
    ...
    value: function _updateSegmentText() {
      ...
      var text = this._segmentText.selectAll("text").data(this._geometry ? this._geometry.lines : []).attr('class', 'segment-measure-text').attr('text-anchor', 'middle').attr('dominant-baseline', 'text-after-edge').attr('transform', function (d) {
        ...
        return _this5._helper.computeLengthBetween(d[0], d[1]);
      });
    ...
    value: function _updateNodeText() {
      ...
      var text = this._nodeText.selectAll("text").data(this._geometry ? this._geometry.nodes : []).attr('class', function (d, i) {
        ...
        return _this6._helper.computePathLength(_this6._geometry.nodes.slice(0, i + 1));
      });
    ...
    value: function _updateSegmentTextPosition(index) {
      ...
      if (index < this._geometry.lines.length) {
        this._segmentText.select('text:nth-child(' + (index + 1) + ')').attr('transform', function (d) {
          ...
          return _this8._helper.computeLengthBetween(_this8._projectionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]), d[1]);
        });
      }
      if (index > 0) {
        this._segmentText.select('text:nth-child(' + index + ')').attr('transform', function (d) {
          ...
          return _this8._helper.computeLengthBetween(d[0], _this8._projectionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]));
        });
      }
    }
  }, {
    ...
    value: function _updateNodeTextPosition(index) {
      ...
      followingNodes.text(function (d, i) {
        return _this9._helper.computePathLength([].concat(_toConsumableArray(_this9._geometry.nodes.slice(0, index)), [_this9._projectionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y])], _toConsumableArray(_this9._geometry.nodes.slice(index + 1, index + 1 + i))));
      });
    }
  }, {
    ...
    value: function _updateArea(i) {
      ...
      if (n > 2) {
        if (i === 0) {
          offset = _helper2.default.computePixelLength(this._projectionUtility.latLngToSvgPoint(this._geometry.nodes[n - 1]), [_d3Selection.event.x, _d3Selection.event.y]);
          area = offset > tolerance ? 0 : this._helper.computeArea([this._projectionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y])].concat(_toConsumableArray(this._geometry.nodes.slice(1, n - 1))));
        } else if (i === n - 1) {
          offset = _helper2.default.computePixelLength([_d3Selection.event.x, _d3Selection.event.y], this._projectionUtility.latLngToSvgPoint(this._geometry.nodes[0]));
          area = offset > tolerance ? 0 : this._helper.computeArea(this._geometry.nodes.slice(0, n - 1));
        } else if (i > 0 && i < n - 1) {
          offset = _helper2.default.computePixelLength(this._projectionUtility.latLngToSvgPoint(this._geometry.nodes[0]), this._projectionUtility.latLngToSvgPoint(this._geometry.nodes[n - 1]));
          area = offset > tolerance ? 0 : this._helper.computeArea([].concat(_toConsumableArray(this._geometry.nodes.slice(0, i)), [this._projectionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y])], _toConsumableArray(this._geometry.nodes.slice(i + 1))));
        } else {
          offset = _helper2.default.computePixelLength(this._projectionUtility.latLngToSvgPoint(this._geometry.nodes[0]), this._projectionUtility.latLngToSvgPoint(this._geometry.nodes[n - 1]));
